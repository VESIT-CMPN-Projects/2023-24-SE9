    ...
    try {
        ...
        if(input.length == 12) {                   //Entered input is Aadhaar number.
            ...
            if (validAadhaar.rows.length != 0) {             //if aadhaar number is valid.
                const existingAadhaar = await db.query(`SELECT * FROM nsp_st_reg WHERE aadhaar_no=$1`,[input]);
...
const verifyAppId = async (req,res) => {
    try {
        ...
        const existingAppId = await db.query(`SELECT * FROM nsp_st_reg WHERE app_id=$1`,[applicationId]);
...
//     try {
//         const query = `SELECT DISTINCT scholarship_name from nsp_po_reg`;
//         const result = await db.query(query);
//         const schemeNames = result.rows.map(row => row.scholarship_name);
//         res.status(200).json({schemeNames});
//     } catch(error) {
...
export {verifyAadhaar,verifyAppId, getStudentProfile};